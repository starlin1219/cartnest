...
class OrderForm(forms.Form):
    ...
    email = forms.EmailField(
        ...
        widget=forms.EmailInput(attrs={
            'class': 'form-control',
            'placeholder': '請輸入E-mail，訂單成立後將寄送訂單資訊給您',
            'required': True
        })
    )

    ship_by = forms.ChoiceField(
        label='寄送方式',
        choices=[('郵寄', '郵寄'), ('宅配', '宅配')],
    ...
    payment = forms.ChoiceField(
        label='付款方式',
        choices=[('信用卡', '信用卡'), ('ATM', 'ATM'), ('貨到付款', '貨到付款')],
        widget=forms.RadioSelect(attrs={
            'class': 'form-check-input',
            'required': True
        })
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 設定 city 選項
        self.fields['city'].choices = [('', '請選擇縣市')] + [(city, city) for city in CITY_DISTRICT_DATA.keys()]

        # 若 POST 傳入 city，依據 city 設定 district options
        if 'city' in self.data:
            city = self.data.get('city')
            districts = CITY_DISTRICT_DATA.get(city, [])
            self.fields['district'].choices = [('', '請選擇區域')] + [(d, d) for d in districts]
        else:
            self.fields['district'].choices = [('', '請先選擇縣市')]
